...
# [이식 시 기계적 수정] 원본(scripts/config.py)은 이 파일 위치에서 parent.parent가
# 저장소 루트였지만, 이 복사본은 backend/app/services/config.py로 한 단계 더 깊어서
# parent 4번(services->app->backend->trading_app)이어야 같은 루트를 가리킨다.
# 계산 로직/값 자체는 원본과 동일 - data/merged/*.parquet, manual_lines.db를
# Streamlit(scripts/) 쪽과 완전히 같은 파일로 공유하기 위한 경로 조정일 뿐이다.
